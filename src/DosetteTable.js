...
function DosetteTable() {
  ...
  const applyFilters = () => {
    ...
    if (filters.intensiteMin !== '') queryParams.append('intensiteMin', filters.intensiteMin);
    if (filters.intensiteMax !== '') queryParams.append('intensiteMax', filters.intensiteMax);
    if (filters.marque !== '') queryParams.append('marque', filters.marque);
    if (filters.pays !== '') queryParams.append('pays', filters.pays);

    fetchDosettes(`http://172.17.0.56:8000/api/filter?${queryParams.toString()}`);
  };

  const showPays = () => {
    setDisplayData(paysOptions.map(pays => ({ id: pays.id, nom: pays.pays, intensite: null, prix: null, marque: null, pays: pays.continent })));
    setIsDisplayingAllDosettes(false);
  };

  const showMarques = () => {
    setDisplayData(marqueOptions.map(marque => ({ id: marque.id, nom: marque.nom, intensite: null, prix: null, marque: 'N/A', pays: 'N/A' })));
    setIsDisplayingAllDosettes(false);
  };
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://172.17.0.56:8000/api/dosettes/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  return (
    <div className="table-container">
      <div className="filters">
        ...
        <button onClick={applyFilters} className="display-button">Appliquer les Filtres</button>
      </div>
      <div className="display-buttons">
        <button className="display-button" onClick={showAllDosettes}>Toutes les Dosettes</button>
        <button className="display-button" onClick={showPays}>Afficher les Pays</button>
        <button className="display-button" onClick={showMarques}>Afficher les Marques</button>
      </div>
      <table>
        <thead>
          <tr>
            ...
            <tr key={item.id}>
              ...
              <td>
                {isDisplayingAllDosettes && (
                  <>
                    <button className="action-button modifier-button" onClick={() => navigate(`/modifier/${item.id}`)}>Modifier</button>
                    <button className="action-button supprimer-button" onClick={() => handleDelete(item.id)}>Supprimer</button>
                  </>
                )}
              </td>
            </tr>
