...
function AdminPanel() {
  ...
  useEffect(() => {
    ...
    fetch('http://172.17.0.56:8000/api/pays-names')
      .then((response) => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ nom: '', intensite: '', prix: '', id_marque: '', id_pays: '' }); // Clear form
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`http://172.17.0.56:8000/api/update/${dosetteId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        setFormData({ nom: '', intensite: '', prix: '', id_marque: '', id_pays: '' }); 
        setDosetteId(null); 
      } else {
        const errorData = await response.json();
  ...
  const fetchDosetteData = async (id) => {
    try {
      const response = await fetch(`http://172.17.0.56:8000/api/showdosette/${id}`);
      if (response.ok) {
    ...
    <div>
      ...
      <form onSubmit={dosetteId ? handleUpdate : handleSubmit}>
        ...
        <input
          ...
          onChange={handleChange}
        />
        <select name="id_marque" value={formData.id_marque} onChange={handleChange}>
